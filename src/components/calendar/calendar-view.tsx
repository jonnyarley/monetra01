...
export function CalendarView() {
  ...
  const yearlyTotals = data?.yearlyTotals || { income: 0, expense: 0, balance: 0 }

  // Get week data for selected month
  const getWeekData = (monthIndex: number) => {
    const weeks: Array<{ week: number; income: number; expense: number; days: string[] }> = []
            ...
            <CardContent className="p-3 md:p-6">
              <div className="flex items-center gap-3 md:gap-4">
                ...
                <div className="min-w-0">
                  <p className="text-xs md:text-sm text-muted-foreground">Receitas do Ano</p>
                  <p className="text-lg md:text-2xl font-bold text-amber-600 truncate">{formatCurrency(yearlyTotals.income)}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-0 shadow-lg bg-gradient-to-br from-rose-500/10 to-pink-500/10">
            <CardContent className="p-3 md:p-6">
              <div className="flex items-center gap-3 md:gap-4">
                ...
                <div className="min-w-0">
                  <p className="text-xs md:text-sm text-muted-foreground">Despesas do Ano</p>
                  <p className="text-lg md:text-2xl font-bold text-rose-600 truncate">{formatCurrency(yearlyTotals.expense)}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-0 shadow-lg">
            <CardContent className="p-3 md:p-6">
              <div className="flex items-center gap-3 md:gap-4">
                ...
                <div className="min-w-0">
                  <p className="text-xs md:text-sm text-muted-foreground">Balanço Anual</p>
                  ...
                  >
                    <CardHeader className="pb-1 p-2 md:p-4">
                      <div className="flex items-center justify-between">
                        ...
                        <Badge 
                          variant={monthData?.balance >= 0 ? "secondary" : "destructive"}
                          className="text-[10px] md:text-xs"
                        >
                          {monthData?.balance >= 0 ? "+" : ""}{formatCurrency(monthData?.balance || 0).replace("R$", "").trim()}
                        </Badge>
                      </div>
                    </CardHeader>
                    <CardContent className="p-2 md:p-4 pt-0">
                          ...
                          return (
                            <div
                              ...
                              className={cn(
                                ...
                                dayData?.transactions?.length && "cursor-pointer hover:ring-2 hover:ring-emerald-500"
                              )}
                              onClick={(e) => {
                                e.stopPropagation()
                      ...
                      {/* Month Summary */}
                      <div className="flex justify-between mt-2 pt-2 border-t text-[10px] md:text-xs">
                        <div className="flex items-center gap-1">
                          <ArrowUpRight className="h-2.5 w-2.5 md:h-3 md:w-3 text-emerald-500" />
                          <span className="text-muted-foreground truncate">{formatCurrency(monthData?.income || 0)}</span>
                        </div>
                        <div className="flex items-center gap-1">
                          <ArrowDownRight className="h-2.5 w-2.5 md:h-3 md:w-3 text-rose-500" />
                          <span className="text-muted-foreground truncate">{formatCurrency(monthData?.expense || 0)}</span>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
