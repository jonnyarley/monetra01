...
import { invalidateTransactionCache, cacheOrFetch, CACHE_TTL } from "@/lib/cache"
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const cacheKey = `user:${decoded.id}:transactions:${JSON.stringify({ page, limit, type, accountId, cardId, categoryId, startDate, endDate, search })}`
    ...
    const result = await cacheOrFetch(
      ...
      { ttl: CACHE_TTL.SHORT } // 1 minuto para transações
    )

    console.log(`[TRANSACTIONS] Encontradas: ${result.transactions.length} transações`)

    // Buscar TODAS as transações para calcular totais (sem paginação) - com cache
    const summaryCacheKey = `user:${decoded.id}:transactions:summary`
    const summary = await cacheOrFetch(
      summaryCacheKey,
      async () => {
    ...
    console.log(`[TRANSACTIONS] Total Receitas: R$ ${summary.totalIncome.toFixed(2)}`)
    console.log(`[TRANSACTIONS] Total Despesas: R$ ${summary.totalExpense.toFixed(2)}`)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rateLimitResponse = await checkRateLimit(request, "WRITE", decoded.id)
    if (rateLimitResponse) return rateLimitResponse
    ...
    await invalidateTransactionCache(decoded.id)
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Invalidar cache após excluir transação
    await invalidateTransactionCache(decoded.id)
